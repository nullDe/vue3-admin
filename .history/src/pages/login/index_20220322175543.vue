...
<template>
    <div class="login-wrap">
        <el-form :model="formData" ref="loginForm" :rules="rules" label-position="left" class="login-container">
            <h3 class="title">用户登录</h3>
            <el-form-item prop="username">
                <el-input type='text' prefix-icon="el-icon-user" v-model="formData.username" placeholder="账号"></el-input>
            </el-form-item>
            <el-form-item prop="password">
                <el-input prefix-icon="el-icon-key" type="password" placeholder="密码" v-model="formData.password" show-password></el-input>
            </el-form-item>
            <el-row>
                <el-col :span="12" style="padding-top:9px;">
                    <el-form-item prop="imgCode">
                        <el-input type="text" 
                                  ...
                                  placeholder="图形验证码" ></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="12" style="padding-left:15px;">
                    <embed ref='captch' src="http://localhost:3000/captcha"  width="180" height="80" />
                </el-col>
            </el-row>
            <el-checkbox class="remember" v-model="rememberPWD">记住密码</el-checkbox>
            <el-form-item style="width:100%;">
                <el-button type="primary" style="width:100%;" :loading="logining" @click="clickLogin('loginForm')">登录</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
        ...
        clickLogin:function(formName){
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    this.logining = true;        
                    //登录请求
                    Axios.post('http://localhost:3000/user/login',this.formData)
                        .then((res)=>{
                            var data = res.data,
                                code = data.code,
                                val = {
                                ...
                                //登录成功
                            if(code == 200){
                                if(this.rememberPWD){ //是否记住密码
                                    setCookie('user', this.formData.username, 1);
                                    setCookie('pwd', this.formData.password, 1);
                                }else{
                                    delCookie('user')
                                    delCookie('pwd')
                                }
                            ...
                            }else if(code == 401){//用户不存在
                                this.logining = false;
                                this.$message.error('用户不存在');
                            }else if(code == 403){//验证码错误
                                this.logining = false;
                            ...
                            t = setTimeout(function(){
                                _self.$refs.captch.src='http://localhost:3000/captcha';
                            },700);
                            }
                        }).catch((err) => {
                            this.logining = false;
